- [🛫 Overview](README.md)
- [🚀 Configure Enviorment](10-config-env.md)
- [⚡️ Lets Deploy](20-lets-deploy.md)
- [🎛 Portal Time](30-portal-time.md)
- [🏁 Conclusion](40-conclusion.md)
- [🔨 Optional](50-optional.md) 

**VPC Infrastructure in IBM Cloud**
- [Lab Sign in](https://ibm.biz/TX24-1561)
- [Block Storage for VPC Documentation](https://cloud.ibm.com/docs/vpc?topic=vpc-block-storage-about)
- [File Storage for VPC Documentation](https://cloud.ibm.com/docs/vpc?topic=vpc-file-storage-planning)
- [Snapshot for VPC Documentation](https://cloud.ibm.com/docs/vpc?topic=vpc-snapshots-vpc-about)
- [Backup for VPC Documentation](https://cloud.ibm.com/docs/vpc?topic=vpc-backup-service-about&interface=ui)
- [Virtual Private Cloud (VPC)](https://cloud.ibm.com/docs/vpc?topic=vpc-about-vpc&interface=ui)

